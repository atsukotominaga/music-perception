---
title: 'Music Perception 2020: Stimuli profiles (random)'
output:
...
I chose stimuli randomly.  
Note: SubNr16 was excluded because of a deviated tempo (see analysis for [teaching-v1.0](https://github.com/atsukotominaga/teaching-v1.0))
...
lf_a_t <- list.files("../random/art_teaching", pattern = "txt")
lf_a_p <- list.files("../random/art_performing", pattern = "txt")
lf_d_t <- list.files("../random/dyn_teaching", pattern = "txt")
lf_d_p <- list.files("../random/dyn_performing", pattern = "txt")

# merge all data files into one
data <- data.table()
for (i in 1:length(lf_a_t)){
  data_i <- fread(file.path("../random/art_teaching", lf_a_t[i]), header = T, sep = ",", dec = ".")
  data_i$Instance <- i
  data <- rbind(data, data_i)
}
for (i in 1:length(lf_a_p)){
  data_i <- fread(file.path("../random/art_performing", lf_a_p[i]), header = T, sep = ",", dec = ".")
  data_i$Instance <- i
  data <- rbind(data, data_i)
}
for (i in 1:length(lf_d_t)){
  data_i <- fread(file.path("../random/dyn_teaching", lf_d_t[i]), header = T, sep = ",", dec = ".")
  data_i$Instance <- i
  data <- rbind(data, data_i)
}
for (i in 1:length(lf_d_p)){
  data_i <- fread(file.path("../random/dyn_performing", lf_d_p[i]), header = T, sep = ",", dec = ".")
  data_i$Instance <- i
  data <- rbind(data, data_i)
}
...
p_ioi <- ggplot(ioi, aes(x = Instance, y = Mean, fill = Condition)) +
  ...
  facet_grid(Skill ~ .) + scale_x_continuous(breaks = seq(1:32)) +
  labs(y = "IOI (ms)", subtitle = "IOI: Each instance (All)")
p_ioi
...
p_ioi_all <- ggplot(ioi_all, aes(x = Skill, y = Mean, fill = Condition)) +
  ...
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD),
                width = .2, position = position_dodge(.9)) +
  geom_hline(yintercept = 188, linetype = "dashed") +
  labs(y = "IOI (ms)", subtitle = "IOI: Averaged (All)")
p_ioi_all
```
...
p_kot <- ggplot(kot, aes(x = Instance, y = Mean, fill = Condition)) +
  ...
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD),
                width = .2, position = position_dodge(.9)) +
  facet_grid(Subcomponent ~ .) + scale_x_continuous(breaks = seq(1:32)) +
