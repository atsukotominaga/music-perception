---
title: "Music Perception: Stimuli profiles (3sd)"
output: html_notebook
---

- Last checked: `r format(Sys.Date(), "%d-%b-%Y")`

I chose stimuli with the IOIs within 3sd randomly.
...
lf_a_t <- list.files("./3sd/art_teaching", pattern = "txt")
lf_a_p <- list.files("./3sd/art_performing", pattern = "txt")
lf_d_t <- list.files("./3sd/dyn_teaching", pattern = "txt")
lf_d_p <- list.files("./3sd/dyn_performing", pattern = "txt")

# merge all data files into one
data <- data.table()
for (i in 1:length(lf_a_t)){
 data_i <- fread(file.path("./3sd/art_teaching", lf_a_t[i]), header = T, sep = ",", dec = ".")
 data_i$Instance <- i
 data <- rbind(data, data_i)
}
for (i in 1:length(lf_a_p)){
 data_i <- fread(file.path("./3sd/art_performing", lf_a_p[i]), header = T, sep = ",", dec = ".")
 data_i$Instance <- i
 data <- rbind(data, data_i)
}
for (i in 1:length(lf_d_t)){
 data_i <- fread(file.path("./3sd/dyn_teaching", lf_d_t[i]), header = T, sep = ",", dec = ".")
 data_i$Instance <- i
 data <- rbind(data, data_i)
}
for (i in 1:length(lf_d_p)){
 data_i <- fread(file.path("./3sd/dyn_performing", lf_d_p[i]), header = T, sep = ",", dec = ".")
 data_i$Instance <- i
 data <- rbind(data, data_i)
}
