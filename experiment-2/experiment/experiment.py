...
"""
This is an experimental programme for Music Perception 2021
"""
...
def trial(expMode, imageFile, midFile, resultsList):
    ...
    ## 1. sheet music
    stimuli = visual.ImageStim(win, image = imageFile, size = [1500, 500])
    stimuli.draw()
...
dlg = gui.DlgFromDict(expInfo, fixed = ["Today"], title = "Music Perception 2021")
if dlg.OK == False:
...
## instruction 2
imageSheet = visual.ImageStim(win, image = imageFile, pos = [0, -50], size = [1500, 500])
...
## instruction 3
inst3 = visual.TextStim(win, pos = [0, 25], font = "Avenir", height = 60, wrapWidth = 1400, text = text_3)
imageSheet.draw()
inst3.draw()
win.flip()
next() # proceed/force quit
...
inst8 = visual.TextStim(win, pos = [0, 0], font = "Avenir", height = 60, wrapWidth = 1400, alignText = "left",
    text = "You will listen to 36 performances (trials). You can take a break between each trial whenever you want.\n\nAny questions? If not,\n\nPress <Space> to start experimental trials")
inst8.draw()
win.flip()
next() # proceed/force quit
