...
\hypersetup{
  ...
  pdflang={en-EN},
  pdfkeywords={perception, expression, pedagogy, expertise, music education},
  hidelinks,
...
A multiple regression analysis was conducted to predict participants' judgments as teaching based on performance features of tempo (IOIs), articulation (KOT for legato parts), dynamics (KV for legato parts) and dynamics contrast (KV-Diff from legato to staccato). The result of the regression indicated that the model explained 19.8 \% of the variance (\emph{F}(4, 31) = 3.16, \emph{p} = .028). It was found that tempo (IOIs; \emph{\(\beta\)} = 0.58, \emph{p} = .011), dynamics (KV; \emph{\(\beta\)} = 1.10, \emph{p} = .018) and dynamics contrast (KV-Diff; \emph{\(\beta\)} = 1.90, \emph{p} = .006) for the legato parts were articulation for the legato parts were significant predictors of participants' judgments as teaching. However, articulation (KOT; \emph{\(\beta\)} = -0.08, \emph{p} = .47) for the legato parts was not a significant predictor.
...
\caption{\label{fig:ioi-2}Experiment 2: Scatter plot showing the correlation between tempo featurea (IOIs) and average participants' judgments as teaching for each recording. Therefore, each dot represents each stimulus. The gray bands display the 95 percent confidence intervals.}\label{fig:plot-ioi-2}
\end{figure}
...
\caption{\label{fig:vel-diff-2}Experiment 2: Scatter plot showing the correlation between dynamics contrast features (KV-Diff) and average participants' judgment as teaching for each stimulus. The gray bands display the 95 percent confidence intervals.}\label{fig:plot-vel-diff-2}
\end{figure}
